...
export class DemoMeetingApp implements AudioVideoObserver, DeviceChangeObserver, ContentShareObserver {
 ...
 /*  metricsDidReceive(clientMetricReport: DefaultClientMetricReport): void {
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    configuration.videoDownlinkBandwidthPolicy = videoPriorityBasedPolicy;
  ...
  async getStatsForOutbound(): Promise<void> {
    let basicReports: {[id: string]: number} =  {};

    let reports = await this.audioVideo.getRTCPeerConnectionStats();
    let duration: number;
    ...
    reports.forEach(report => {
      if (report.type === 'outbound-rtp') {
        ...
        this.log(`report is bound to ssrc ${report.ssrc}`);
    ...
    await new TimeoutScheduler(1000).start(() => {
      this.audioVideo.getRTCPeerConnectionStats().then((reports) => {
        if (reports != null) {
        reports.forEach(report => {
          if (report.type === 'outbound-rtp') {
...
videoTilePin(e:any) {
  ...
  videoTileDidUpdate(tileState: VideoTileState): void {
    ...
    pauseButtonElement.addEventListener('click', this.videoTilePin);
  ...
  videoTileWasRemoved(tileId: number): void {
    ...
    this.log(`video tile removed: ${tileIndex}`);
    const pauseButtonElement = document.getElementById(`video-pause-${tileIndex}`) as HTMLButtonElement;
    pauseButtonElement.removeEventListener('click', this.videoTilePin);
  ...
  pinnedTiles(): number[] {
    ...
    for (const attendeeId in this.roster) {
      if (this.roster[attendeeId].active) {
        let tile = this.tileIdForAttendeeId(attendeeId);
        if (!!tile) {
          pinned.push(tile);
        }
      }
    ...
    if (this.pinnedTiles().length !== 0) {
      ...
      for (const attendeeId in this.roster) {
        if (this.roster[attendeeId].active) {
          ...
          videoPreferences.push(new VideoPreference(attendeeId, 2));
        }
      }
    }
  ...
  layoutVideoTiles(): void {
    ...
    const pinnedTiles = this.pinnedTiles();
    if (this.activeSpeakerLayout && hasVisibleActiveTile) {
      this.layoutVideoTilesActiveSpeaker(visibleTileIndices, activeTileId);
    } else if (pinnedTiles.length > 0 && hasVisibleActiveTile) {
      this.layoutVideoTilesPinned(visibleTileIndices, pinnedTiles);
    } else {
  ...
  layoutVideoTilesPinned(visibleTileIndices: number[], pinnedTiles: number[]): void {
    ...
    const pinnedWidth = activeWidth/((pinnedTiles.length == 1)?1:2);
    const pinnedHeight = pinnedWidth / widthToHeightAspectRatio;
    let othersWidth = 0;
    let othersHeight = 0;
    if (othersCount > 0) {
      othersWidth = width / othersCount;
      othersHeight = width / widthToHeightAspectRatio;
    ...
    for (let i = 0; i < visibleTileIndices.length; i++) {
      ...
      let x = 0,
        ...
        w = 0,
        h = 0;
      if (pinnedTiles.indexOf(tileId) !== -1) {
        x = 0 + (pinnedIndex%2)*pinnedWidth;
        y = Math.trunc(pinnedIndex/2)*pinnedHeight; //activeYOffset;
        w = pinnedWidth;
        h = pinnedHeight;
        pinnedIndex += 1;
      } else {
